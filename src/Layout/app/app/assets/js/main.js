...
$(function() {
  ...
  QUICKCHAT.global = {
    ...
    init: function() {
      QUICKCHAT.global.deviceSize();
      QUICKCHAT.global.layout();
      QUICKCHAT.global.animsition();
    },
    ...
    layout: function() {
      var defaultHeaderScheme = 'scheme-default',
          ...
          defaultAudioVisibility = 'audio-hidden',
          defaultVideoVisibility = 'video-hidden',
          defaultAppClasses = 'scheme-default default-scheme-color header-fixed aside-fixed rightbar-hidden video-hidden audio-hidden';
      ...
      $toggleAudioEl.on('click', function() {
        if ($body.hasClass('audio-hidden')) {
          $body.removeClass('audio-hidden').addClass('audio-show');
        } else {
          $body.removeClass('audio-show').addClass('audio-hidden');
        }
      });
      
      $toggleVideoEl.on('click', function() {
        if ($body.hasClass('video-hidden')) {
          $body.removeClass('video-hidden').addClass('video-show');
        } else {
          $body.removeClass('video-show').addClass('video-hidden');
        }
      });
  ...
  QUICKCHAT.navbar = {
    ...
    init: function() {
      QUICKCHAT.navbar.menu();
      QUICKCHAT.navbar.ripple();
      QUICKCHAT.navbar.removeRipple();
      QUICKCHAT.navbar.collapse();
      QUICKCHAT.navbar.offcanvas();
    },
  ...
  QUICKCHAT.tiles = {
    ...
    init: function() {
      QUICKCHAT.tiles.toggle();
      QUICKCHAT.tiles.refresh();
      QUICKCHAT.tiles.fullscreen();
      QUICKCHAT.tiles.close();
    },
  ...
  QUICKCHAT.extra = {
    ...
    init: function() {
      QUICKCHAT.extra.sparklineChart();
      QUICKCHAT.extra.slimScroll();
      QUICKCHAT.extra.daterangePicker();
      QUICKCHAT.extra.easypiechart();
      QUICKCHAT.extra.chosen();
      QUICKCHAT.extra.toggleClass();
      QUICKCHAT.extra.colorpicker();
      QUICKCHAT.extra.touchspin();
      QUICKCHAT.extra.datepicker();
      QUICKCHAT.extra.animateProgress();
      QUICKCHAT.extra.counter();
      QUICKCHAT.extra.popover();
      QUICKCHAT.extra.tooltip();
      QUICKCHAT.extra.splash();
      QUICKCHAT.extra.lightbox();
    },
    ...
    daterangePicker: function() {
      ...
      if ($pickDateEl.length > 0) {
        $pickDateEl.each(function() {
          ...
          element.find('span').html(moment().subtract(29, 'дней').format('MMMM D, YYYY') + ' - ' + moment().format('MMMM D, YYYY'));
          ...
          element.daterangepicker({
            ...
            ranges: {
              ...
              'Вчера': [moment().subtract(1, 'дня'), moment().subtract(1, 'дней')],
              'Последние 7 дней': [moment().subtract(6, 'дней'), moment()],
              'Последние 30 дней': [moment().subtract(29, 'дней'), moment()],
              'Прошлый месяц': [moment().subtract(1, 'месяц').startOf('месяц'), moment().subtract(1, 'месяц').endOf('месяц')]
            },
            opens: 'left',
            drops: 'down',
  ...
  QUICKCHAT.isMobile = {
    ...
    any: function() {
      return (QUICKCHAT.isMobile.Android() || QUICKCHAT.isMobile.BlackBerry() || QUICKCHAT.isMobile.iOS() || QUICKCHAT.isMobile.Opera() || QUICKCHAT.isMobile.Windows());
    }
  };
  ...
  QUICKCHAT.documentOnResize = {

		init: function(){

      var t = setTimeout( function(){

        QUICKCHAT.documentOnReady.setSidebar();
        QUICKCHAT.navbar.removeRipple();
  ...
  QUICKCHAT.documentOnReady = {

		init: function(){
      QUICKCHAT.global.init();
			QUICKCHAT.header.init();
      QUICKCHAT.navbar.init();
      QUICKCHAT.documentOnReady.windowscroll();
      QUICKCHAT.tiles.init();
      QUICKCHAT.extra.init();
      QUICKCHAT.documentOnReady.setSidebar();
		},
  ...
  var $window = $(window),
      ...
      $toggleVideoEl = $('.toggle-video-sidebar'),
      $toggleAudioEl = $('.toggle-audio-sidebar'),
      $pickDateEl = $('.pickDate'),
  ...
  // initializing
  //!!!!!!!!!!!!!
  $(document).ready( QUICKCHAT.documentOnReady.init );
  $window.load( QUICKCHAT.documentOnLoad.init );
  $window.on( 'resize', QUICKCHAT.documentOnResize.init );
